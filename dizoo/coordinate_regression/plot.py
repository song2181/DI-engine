...
from .dataset import CoordinateRegression
...
def eval(
        train_dataloader: torch.utils.data.DataLoader, test_dataloader: torch.utils.data.DataLoader, policy
) -> np.ndarray:
    ...
    for batch in tqdm(test_dataloader):
        input, target = batch
        prediction = policy.forward(input)['action'].cpu().numpy()
...
# def plot_image(args: Args):
#     plot_dir = pathlib.Path(args.plot_dir)
#     plot_dir.mkdir(parents=True, exist_ok=True)

#     experiment = Experiment(identifier=args.experiment_name, ).assert_exists()
